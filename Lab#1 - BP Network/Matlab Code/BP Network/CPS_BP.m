function [ net, REF_PRED_FIX ] = CPS_BP( net )
...
REF_TRAIN = zeros(H, 3);
for i = 1 : H
    pos = uint8(floor(mod(TRAIN(i, 1)-1, 6)/2) + 1);
    REF_TRAIN(i, pos) = 1;
end
...
if nargin == 0
    net = newff(minmax(TRAIN), [500, 500, 1000, 3], { 'logsig' 'logsig' 'logsig' 'logsig' }, 'trainscg');
end
...
REF_TEST = zeros(H, 3);
for i = 1 : H
    pos = uint8(floor(mod(TEST(i, 1)-1, 6)/2) + 1);
    REF_TEST(i, pos) = 1;
end
